...
export default function RegisterPage() {
  ...
  const handleSocialRegister = async (provider: "google" | "facebook") => {
    ...
    try {
      // Normally, you would redirect to OAuth provider here
      // For this implementation, we'll simulate receiving user data from provider

      // In a real implementation, this would be data from the OAuth provider after consent
      const mockSocialUserData = {
        fullName: provider === "google" ? "Google User" : "Facebook User",
        email: `${provider.toLowerCase()}_user_${Date.now()}@example.com`,
      };

      // Call the social auth API
      const response = await fetch(`/api/auth/${provider}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(mockSocialUserData),
      });
      ...
      // Social logins typically don't require email verification
      router.push("/");
    } catch (error) {
  ...
  const CriteriaItem = ({ met, label }: { met: boolean; label: string }) => (
    <div className="flex items-center space-x-2">
      ...
      <span
        className={`text-sm ${
          met ? "text-green-500" : "text-gray-500 dark:text-gray-400"
        }`}
      >
        {label}
      </span>
  ...
  return (
    <AuthLayout
      ...
      subtitle="Sign up to get started with DealDash"
    >
      {registerError && (
        <motion.div
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3">
              ...
              {privacyError && (
                <p className="mt-1 text-sm text-red-500">{privacyError}</p>
              )}
            </div>
          </div>
        </div>
