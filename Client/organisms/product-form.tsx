...
export function ProductForm({ product = null, isOpen, onClose, onSave }) {
  ...
  const handleRemoveImage = (imageId, variantIndex = -1) => {
    if (variantIndex === -1) {
      ...
      const updatedVariants = [...formData.variants]
      updatedVariants[variantIndex].images = updatedVariants[variantIndex].images.filter((img) => img.id !== imageId)
  ...
  const handleSave = () => {
    if (!validateForm()) {
      ...
      if (errorFields.some((field) => ["name", "sku", "price", "stock", "categoryId", "brandId"].includes(field))) {
          ...
          <TabsContent value="general" className="space-y-4">
            ...
            <Card>
              <CardContent className="p-6 space-y-4">
                ...
                <div className="flex items-center space-x-2">
                  <Switch id="hasVariants" checked={formData.hasVariants} onCheckedChange={handleToggleVariants} />
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="text-sm text-gray-500 mt-4 flex items-center">
                  ...
                  You can add up to 4 images. First image will be used as the main product image.
                </div>
              </CardContent>
            </Card>
          </TabsContent>
                ...
                {!formData.hasVariants ? (
                  <div className="text-center py-8 border rounded-md bg-gray-50 dark:bg-gray-800">
                    <p className="text-gray-500 dark:text-gray-400 mb-4">
                      This product doesn't have variants. Enable variants in the General tab if you need to create
                      options like different sizes or colors.
                    </p>
                    <Button
                      variant="outline"
                      onClick={() => {
