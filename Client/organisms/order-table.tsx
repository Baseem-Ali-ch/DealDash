...
interface OrderTableProps {
  ...
  onPaymentStatusChange: (orderId: string, newStatus: PaymentStatus) => Promise<void>
}

export function OrderTable({ orders: initialOrders, onStatusChange, onPaymentStatusChange }: OrderTableProps) {
  ...
  const exportOrders = () => {
    ...
    alert("Order data export initiated. The file will be available for download shortly.")
  }
        ...
        <Sheet open={!!selectedOrder} onOpenChange={(open) => !open && setSelectedOrder(null)}>
          <SheetContent className="sm:max-w-md md:max-w-lg lg:max-w-xl overflow-y-auto">
            <SheetHeader className="mb-4">
              ...
              <SheetDescription>Placed on {new Date(selectedOrder.date).toLocaleDateString()}</SheetDescription>
            </SheetHeader>
            <OrderDetail
              order={selectedOrder}
              onStatusChange={(newStatus) => handleStatusChange(selectedOrder.id, newStatus)}
              onPaymentStatusChange={(newStatus) => handlePaymentStatusChange(selectedOrder.id, newStatus)}
            />
          </SheetContent>
        </Sheet>
      )}
